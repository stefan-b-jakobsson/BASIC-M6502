...
def format_line(sline, tline, buf):
    pos = 1

    # Label
    index = 0
    label = re.search(r"^([A-Z]{1}[A-Z|0-9|%|$|.]*\:)", buf)
    if label:
        print (label.group(1), end="")
        index = len(label.group(1))
    ...
    if hascomment:
        if sline.find(";") > 3:
            # Comment not at beginning of source line, indent to column 41
            pos += x
            while pos < 41:
                pos += 8
                print ("\t", end="")
